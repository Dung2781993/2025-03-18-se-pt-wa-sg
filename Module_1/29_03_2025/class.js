...
// We will access via key (data)

//Get the image that will belong to Salman Khan - Lane - Joel - Kelvin

// We want to get the images details but we will need to consider or use the user details to check the name is matching or not


function getImageDetailsByUser(fullName, imageArr) {
  // iterate through the list of image and for each image, we need to get
  // user_details => use the user_details (full_name) to compare with the expected fullName
  // if it match then we will get the image_details => add to the return list

  let result = [];
  for(let i = 0; i < imageArr.length; i++) {
    // let userDetails = imageArr[i]?.user ? imageArr[i]?.user : 'Not found'; // Only get the User Details if the key - user exists
    let userDetails = '';
    if (imageArr[i]?.user) {
      userDetails = imageArr[i]?.user;
    } else {
      userDetails = 'Not found';
    }

    if (userDetails?.full_name === fullName && imageArr[i]?.images) { // Check the full_name of the userDetails is matching with the 
    // expected fullName or not and checking the author has the image album or not
      result.push(imageArr[i].images)
    }
  }
  return result;
}
